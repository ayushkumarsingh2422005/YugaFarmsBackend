...
type OrderController = {
  sanitizeOutput: (entity: unknown, ctx: Context) => Promise<unknown>;
  transformResponse: (entity: unknown) => unknown;
};

async function resolveAuthenticatedUser(ctx: {
...
async function createRazorpayIntentHandler(ctx: Context) {
...
async function confirmRazorpayPaymentHandler(ctx: Context, controller: OrderController) {
  const user = await resolveAuthenticatedUser(ctx);
  if (!user) {
    return ctx.unauthorized('You must be authenticated');
  }
  ...
  if (existing) {
    const sanitizedEntity = await controller.sanitizeOutput(existing, ctx);
  ...
  const sanitizedEntity = await controller.sanitizeOutput(order, ctx);
  return controller.transformResponse(sanitizedEntity);
}
...
export default factories.createCoreController('api::order.order', ({ strapi }) => ({
  async create(ctx) {
    ...
    if (body.razorpayConfirm === true) {
      return confirmRazorpayPaymentHandler(ctx, this as unknown as OrderController);
    }
    ...
    if (data.razorpayIntentOnly === true) {
      ...
      return createRazorpayIntentHandler(ctx);
    }
